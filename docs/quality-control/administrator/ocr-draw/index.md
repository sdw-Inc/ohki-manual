...
1. 必須項目を入力して「登録」を選択して完了です。
    ...
    {: .warning }
    ファイルサイズが100MByteを超える場合は、図面登録に失敗する可能性があるため分けて登録してください。
...
1. OCR読取を行いたい図面を選択します。デフォルトでは全て選択となっています。
    ...
    {: .warning }
    読取箇所が違う図面はチェックを外すなどして分けてOCR読取を行なってください。
...
1. OCR読取を行なった部材名と図面番号の手動修正を行います。確認したら「次へ」を選択します。
    ...
    <table><tr><td>
    <img src="../../../../assets/images/quality-control/administrator/ocr-draw/18.png" width="80%">
    </td></tr></table>
...
1. 入力した図面番号と部材名が一覧で表示されます。  
    ...
    <table><tr><td>
    <img src="../../../../assets/images/quality-control/administrator/ocr-draw/20.png" width="75%">
    </td></tr></table>

    - <span style="color:red ; ">①</span>「成形後図面番号、成形後部材名」  
    　前画面でOCR読取、手動修正した内容が表示されます。
    - <span style="color:red ; ">②</span>「紐づく製品番号」  
    　成形後部材名と製品マスタの製品番号を照合し、  
    　条件に一致する製品マスタの製品番号が表示されます。
    - <span style="color:red ; ">③</span>「詳細」  
    　クリックで各図面を登録する対象の製品番号と図面IDが表示されます。
    - <span style="color:red ; ">④</span>「確認欄」  
...
1. 「詳細」クリックで図面をアップする対象の製品番号が一覧で表示されます。アップする図面IDや対象製品を変更することが可能です。  
    ...
    {: .note }
    デフォルトのアップ対象図面IDは、図面が登録されていない図面IDに左詰めとなっています。
...
1. [出図・変更図一覧]画面の右上にある「アップロード」を選択して完了です。アップロード後は参照画面に切り替わります。
    ...
    また、製品詳細画面の図面タブに、以下の図面登録情報が表示されます。
    - 図面登録日
    - 図面タイプ
    - 図面番号
    ...
    pdf名は、[製品番号.pdf]で登録されます。拡大ボタンクリックで確認できます。
    <table><tr><td>
